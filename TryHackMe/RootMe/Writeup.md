...
A quick check on `/css` and `/js` doesn't net anything useful (besides a pretty neat js function for typewriter-like typing) so we move to `/uploads` and `/panel`. Before though, one more question answered!
...
Would ya look at that! `/panel` is a file uploader! Let me just send my whole `cheatsheet.txt`...
