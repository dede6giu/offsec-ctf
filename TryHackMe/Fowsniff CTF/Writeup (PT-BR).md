...
Bem, tem um `http`, um `ssh`, e um sistema de emails com `pop3` e `imap`. No `http` temos apenas um aviso de vazamento de dados... Usualmente, para a solução desta máquina, seria necessário acessar o twitter marcado na página e eventualmente encontrar um pastebin com os hashes. Contudo, o pastebin foi derrubado num falso positivo, então segui usando o `.txt` fornecido pelo próprio autor: [fowsniff.txt](https://github.com/berzerk0/Fowsniff/blob/main/fowsniff.txt).
...
Com isso segui para o email, e tentei cada combinação válida, torcendo para que alguém não tenha ainda trocado sua senha...
...
Com o reverse shell[^rv] fornecido pelo próprio desafio, eu substitui o script para executar `python3`.[^py] Eu também coloquei o `netcat`[^nc] para ouvir na porta `1234` e finalmente tentei realizar o login no ssh novamente:
