...
Como de usual, a primeira coisa a se fazer é colocar o IP da máquina como um host de DNS em `/etc/hosts` para fácil acesso. Escolhi `ectf.net`, e testando para verificar se tudo está funcionando:
...
Eu não acho que terão resultados muito interessantes. Enquanto o `gobuster` roda, decidi explorar o `ftp` como anonymous para verificar o que estava disponível:
...
Apesar de ter o usuário, sem nenhuma indicação de senha (e te garanto que URI não funcionou) decidi procurar por informações em outro lugar.
